...
We compare the functionality of \textbf{SCAPE-ZK} with representative schemes, namely XAuth~\cite{b6}, SSL-XIoMT~\cite{b8}, and the Cross-Domain Identity Authentication Scheme for the IIoT Identification Resolution System Based on Self-Sovereign Identity~\cite{b30} (abbreviated as the IIoT SSI identity-resolution scheme). These schemes are selected due to their relevance in decentralized authentication and secure cross-domain data sharing for IoT, IIoT, and healthcare environments.
...
\hline
IIoT SSI~\cite{b30} & $\checkmark$ & $\times$ & $\checkmark$ & $\times$ & $\times$ & $\checkmark$ \\
\hline
...
We evaluate the computational cost of SCAPE-ZK against XAuth~\cite{b6}, SSL-XIoMT~\cite{b8}, and the IIoT SSI identity-resolution scheme~\cite{b30}, focusing on the online access workflow, including authentication, authorization, and secure data retrieval. The analysis decomposes costs into fundamental cryptographic operations and distinguishes between per-request overhead and system-level scalability.
...
From an encryption perspective, SCAPE-ZK employs a hybrid symmetric and CP-ABE design while eliminating ECC-based key exchange and avoiding pairing-based cryptographic operations during the online phase. This results in a more lightweight and practical execution model compared to SSL-XIoMT and the IIoT SSI identity-resolution scheme~\cite{b30}.

While Table~\ref{tab:cost-unified} focuses on per-request overhead, Table~\ref{tab:scalability} highlights system-level scalability. XAuth and SSL-XIoMT incur linear verification cost $n \cdot T_{\mathsf{zk}}^{v}$ due to independent verification of each request. The IIoT SSI identity-resolution scheme~\cite{b30} improves efficiency through batch authentication; however, its total verification cost remains linear due to aggregation overhead and group operations, limiting scalability under high concurrency.
...
To quantify this scalability trend, we instantiate the on-chain terms in Table~\ref{tab:scalability} using locally measured primitive timings rather than arbitrary normalized constants. The primitive calibration gives $T_{\mathsf{hash}}=0.002215$ ms for SHA-256 over a compact metadata input, $T_{\mathsf{grp}}=0.456854$ ms for a BLS12-381 group operation, $T_{\mathsf{pair}}=14.724512$ ms for a BLS12-381 pairing, and $T_{\mathsf{zk}}^{v}=13.293$ ms for Groth16 request-proof verification. Under this primitive-calibrated Table~\ref{tab:scalability} model, SCAPE-ZK remains constant at $14.724512$ ms for each aggregate authorization batch, while XAuth and SSL-XIoMT grow linearly as $nT_{\mathsf{hash}}$ and the IIoT SSI identity-resolution scheme grows linearly with additional ZKP, pairing, and group-operation terms. Because hashing is substantially cheaper than a pairing, the hash-only baselines are lower at small $n$; however, SCAPE-ZK crosses below the $nT_{\mathsf{hash}}$ model after approximately $6{,}648$ aggregated requests and remains lower across high-concurrency hospital-network workloads from $n=10{,}000$ to $n=50{,}000$. These values are primitive-calibrated analytical costs derived from Table~\ref{tab:scalability}, not full reimplementations of the baseline systems.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_fabric_tps_vs_load.png}
\caption{SCAPE-ZK Fabric throughput versus submitted transaction load. The graph is generated from ten Hyperledger Caliper trials and includes the four ledger write operations: Register, VerifyProof, Revoke, and UpdateCred.}
\label{fig:fabric-tps-load}
\end{figure}

As shown in Fig.~\ref{fig:fabric-tps-load}, the four write operations follow a similar scaling pattern as the submitted load increases. At 100 transactions, the write throughput is approximately 19--20 TPS: Register reaches $19.34 \pm 3.26$ TPS, VerifyProof reaches $20.07 \pm 3.30$ TPS, Revoke reaches $19.87 \pm 3.03$ TPS, and UpdateCred reaches $20.28 \pm 1.49$ TPS. At 500 transactions, the same operations increase to approximately 131 TPS, with Register at $131.33 \pm 2.51$ TPS, VerifyProof at $130.99 \pm 4.93$ TPS, Revoke at $130.83 \pm 4.76$ TPS, and UpdateCred at $131.65 \pm 5.06$ TPS. At 1000 transactions, the write operations remain closely grouped: Register reaches $215.15 \pm 14.24$ TPS, VerifyProof reaches $221.19 \pm 8.03$ TPS, Revoke reaches $221.20 \pm 13.61$ TPS, and UpdateCred reaches $221.56 \pm 10.47$ TPS.
...
The read-only RecordExists query is not included in Fig.~\ref{fig:fabric-tps-load} because it does not follow the same write-transaction path. Its measured throughput is higher: $112.98 \pm 1.41$ TPS at 100 transactions, $447.05 \pm 15.10$ TPS at 500 transactions, and $702.38 \pm 38.86$ TPS at 1000 transactions. This result is expected because RecordExists avoids ordering, validation, commit, and world-state update overhead. The separation between read and write performance supports the intended SCAPE-ZK design, where expensive cryptographic work is performed off-chain and the blockchain stores compact authorization, revocation, and audit state.

Average latency also remains low at the 1000-transaction load. Register records $89 \pm 12$ ms, VerifyProof records $87 \pm 7$ ms, Revoke records $86 \pm 8$ ms, and UpdateCred records $92 \pm 8$ ms. RecordExists completes in $10 \pm 0$ ms on average. These values show that the tested Fabric deployment can sustain hundreds of SCAPE-ZK ledger write operations per second while keeping average write latency below 100 ms at the highest tested load.
